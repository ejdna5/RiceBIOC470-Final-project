...
fid=fopen('AllExpressionData.csv','w');
printString='Entrez_ID,Gene,mouse_scBAT_count,mouse_iBAT_counts,mouse_WAT_count,BAT1_int_Wu,BAT2_int_Wu,BAT3_int_Wu,beige1_int_Wu,beige2_int_Wu,beige3_int_Wu,Human_Genbank_Accession,Human_Gene,Human_scBAT_FPKM,Human_WAT_FPKM'; 
...
for i=1:length(HumanMouse_combinedRNAseq_table.Mouse_Entrez)
    ...
    if index~=0 %only include genes in both datasets
        ...
        printString=[ '\n' num2str(mID(y)) ',' char(mGene(y)) ',' num2str(mSCBATcount(y)) ',' num2str(mIBATcount(y)) ',' num2str(mWATcount(y)) ',' num2str(mbrown1int(y)) ',' num2str(mbrown2int(y)) ',' num2str(mbrown3int(y)) ',' num2str(mbeige1int(y)) ',' num2str(mbeige2int(y)) ',' num2str(mbeige3int(y)) ',' char(hID(y)) ',' char(hGene(y)) ',' num2str(hSCBATcount(y)) ',' num2str(hWATcount(y)) ];
        fprintf(fid,printString); 
        y=y+1;
    end
end
